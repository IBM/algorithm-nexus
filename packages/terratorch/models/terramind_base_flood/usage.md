# Using the TerraMind Base Flood Model

This guide provides instructions on how to use the TerraMind Base Flood model
for geospatial flood segmentation tasks using vLLM serving.

## Overview

The TerraMind Base Flood model (`ibm-esa-geospatial/TerraMind-base-Flood`) is a
geospatial foundation model designed for flood detection and segmentation in
Earth observation imagery. It processes multi-modal satellite data including DEM
(Digital Elevation Model), Sentinel-1 RTC, and Sentinel-2 L2A imagery. The model
can be served using vLLM with TerraTorch integration for efficient inference.
...
```bash
vllm serve ibm-esa-geospatial/TerraMind-base-Flood \
...
if response.status_code == 200:
    ...
    # Save the prediction
    output_path = Path.cwd() / "terramind_flood_prediction.tiff"

...
- Can optionally register model-specific benchmark submissions for benchmarking workflows
...
one sub-folder for each model. Each model folder must contain a `model.yaml` file
describing the model metadata and optional vLLM integration. A `usage.md` file
...
Benchmark configuration is kept separate from `models/<model-name>/model.yaml`
and from the `packages/` tree entirely. Benchmark experiments and submissions
live under the top-level `experiments/` directory:

```text
experiments/
└── <experiment-name>/
    ├── experiment_package.yaml  # Required: experiment package specifier
    ├── bindings/                # Optional: benchmark binding YAML files
    └── submissions/
        └── <submission-name>/
            └── space.yaml       # Required: ADO discoveryspace definition
```

Each `experiments/<experiment-name>/experiment_package.yaml` declares the Python package
